...
//! Like I said, we can do anything we want there. Even a non-linear reward curve to disincentivize
//! whales.
//!
//! ## Payout Mechanism
//!
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn associate_native_identity(
			...
			// Check that the origin is the one able to associate the reward address
			...
			Self::deposit_event(Event::InitialPaymentMade(reward_account.clone(), first_payment));
		...
		pub fn claim(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
			...
			}; //first_paid is recorded here as part of the payable_amount, but that has been paid already at initialize_reward_vec()
		...
		pub fn complete_initialization(
			...
			ensure!(initialized == false, Error::<T>::RewardVecAlreadyInitialized);
		...
		pub fn initialize_reward_vec(
			...
			let incoming_rewards: BalanceOf<T> = rewards
				.iter()
				.fold(0u32.into(), |acc: BalanceOf<T>, (_, _, reward)| acc + *reward);

			info!("incoming_rewards: {:?}", incoming_rewards);
